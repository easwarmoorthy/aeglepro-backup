...
async function uploadFile (backupFile) {
  try {
    ...
    const oldFile = date.toISOString().substring(0, 10) + '.gz'
...
const takePGBackup = async () => {
  ...
  await execute(`export PGPASSWORD=${process.env.PGPASSWORD}; pg_dump -U ${username} -h ${dbHost} -p ${dbPort} -f ${backupFile} -F t -d ${database}`)
    .then(async () => {
